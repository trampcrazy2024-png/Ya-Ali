...
import { recordTelemetry } from './services/privacyTelemetry';
...
const FORMAT_KEY='yaali_local_model_format';
...
export function getLocalModelFormatOverride():LocalModelFormat|undefined{try{const v=localStorage.getItem(FORMAT_KEY)||'';return v as LocalModelFormat||undefined}catch{return undefined}}
export function setLocalModelFormatOverride(format?:LocalModelFormat){try{format?localStorage.setItem(FORMAT_KEY,format):localStorage.removeItem(FORMAT_KEY)}catch{}}
...
export async function pickLocalModel():Promise<LocalModelStatus>{
  ...
  try{
    ...
    if(r?.path){
      const nativeFormat=String(r.format||'');
      const info=detectLocalModelFormat(String(r.path));
      const finalFormat=(nativeFormat&&nativeFormat!=='unknown'?nativeFormat:info.format) as LocalModelFormat;
      if(finalFormat==='unknown') return {loaded:false,error:'فرمت مدل ناشناخته است. فرمت‌های معتبر: GGUF، ONNX/ONNX bundle ZIP، PTE، Safetensors، PyTorch و TFLite.'};
      setLocalModelPath(r.path); setLocalModelFormatOverride(finalFormat); const finalInfo=detectLocalModelFormat(finalFormat==='onnx'?'model.onnx':r.path); return {loaded:false,imported:true,engineReady:false,path:r.path,name:r.name,sizeBytes:r.sizeBytes,format:finalFormat,execution:finalInfo.execution,directOnDevice:finalInfo.directOnDevice};
    }
    return {loaded:false,error:'مدل انتخاب نشد.'};
  }catch(e:any){return {loaded:false,error:e?.message||String(e)}}
}
export async function listLocalModels():Promise<LocalModel[]>{ if(!Capacitor.isNativePlatform()) return []; try{ const r=await NativeLocalAI.listModels(); const selected=getLocalModelPath(); const rows=Array.isArray(r?.models)?r.models.map((m:any)=>({...m,loaded:!!m.loaded||m.path===selected})):[]; const current=rows.find((m:any)=>m.path===selected); if(current?.format) setLocalModelFormatOverride(String(current.format) as LocalModelFormat); return rows; }catch{return []} }
export async function deleteLocalModel(path:string){ if(!Capacitor.isNativePlatform()) return false; try{ const r=await NativeLocalAI.deleteModel({path}); if(getLocalModelPath()===path) { setLocalModelPath(''); setLocalModelFormatOverride(); } return !!r?.deleted; }catch{return false} }
export async function loadLocalModel(path=getLocalModelPath()):Promise<LocalModelStatus>{
  ...
  const override=path===getLocalModelPath()?getLocalModelFormatOverride():undefined; const format=override?detectLocalModelFormat(override==='onnx'?'model.onnx':override==='pte'?'model.pte':override==='litertlm'?'model.litertlm':path):detectLocalModelFormat(path);
...
export async function localModelStatus():Promise<LocalModelStatus>{
  const saved=getLocalModelPath(); const savedOverride=getLocalModelFormatOverride(); const savedInfo=savedOverride?detectLocalModelFormat(savedOverride==='onnx'?'model.onnx':savedOverride==='pte'?'model.pte':savedOverride==='litertlm'?'model.litertlm':saved):detectLocalModelFormat(saved);
...
export async function localChat(messages:{role:string;content:string}[]):Promise<string>{
  ...
  if(format==='onnx'||format==='pte'||format==='litertlm'){
    ...
    try {
      ...
      const text=String(r?.text||'').trim(); if(!text)throw new Error('Edge runtime پاسخ خالی برگرداند.'); recordTelemetry('inference',performance.now()-started,true); return text;
    } catch(e) { recordTelemetry('inference',performance.now()-started,false); throw e; }
  }
  const timeout=window.setTimeout(()=>{void cancelLocalGeneration()},90000);
  const started=performance.now();
  try{const r=await NativeLocalAI.generate({messages,maxTokens:384,temperature:0.65});const text=String(r?.text||'').trim();if(!text)throw new Error('مدل محلی پاسخ خالی برگرداند. مدل Instruct/Chat را انتخاب کنید.');recordTelemetry('inference',performance.now()-started,true);return text;}catch(e){recordTelemetry('inference',performance.now()-started,false);throw e;}finally{window.clearTimeout(timeout)}
}
