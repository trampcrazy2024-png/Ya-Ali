#!/usr/bin/env bash
# رفع TS2322: jsonArray باید EndpointProtocol[]/EndpointCapability[] برگرداند نه string[]
# اجرا از ریشه‌ی ریپو: cd /workspaces/Ya-Ali && bash fix_endpointPersistence_types.sh
set -e
cd apps/mobile/src/services

cat > endpointPersistence.ts <<'EOF'
...
echo "--- اندازه بعد از بازنویسی ---"
wc -c endpointPersistence.ts
echo "✅ endpointPersistence.ts با jsonArray عمومی (generic) بازنویسی شد."
